...
const bartersSlice = createSlice({
  ...
  reducers: {
    ...
    deleteBarter(state, action: { payload: string }) {
      ...
      state.bartersData = state.bartersData.filter((barter) => barter.id !== barterId);
...
export const { setBartersData, setLocationsData, addBarter, deleteBarter, updateLocation } = bartersSlice.actions;
