...
const tiersSlice = createSlice({
  ...
  reducers: {
    ...
    deleteTier(state, action: { payload: string }) {
      state.tiersData = state.tiersData.filter(tier => tier.id !== action.payload);
    },
    editTierReducer(state, action: { payload: { id: string; updatedTier: Partial<Tier> } }) {
...
export const { setTiersData, addTier, deleteTier, editTierReducer } = tiersSlice.actions;
